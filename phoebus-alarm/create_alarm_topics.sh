...
for topic in "$1"
do
    /opt/epics-tools/services/nasa/kafka/kafka_2.13-3.6.2/bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --replication-factor 1 --partitions 1 --topic $topic
    /opt/epics-tools/services/nasa/kafka/kafka_2.13-3.6.2/bin/kafka-configs.sh --bootstrap-server localhost:9092 --entity-type topics --alter --entity-name $topic \
